...
void keypad_init(void) {
    P5->DIR = 0;
    P5->REN = 0xE0;     /* enable pull resistor for column pins */
    P5->OUT = 0xE0;     /* make column pins pull-ups */
}
...
char keypad_getkey(void) {
    ...
    P5->DIR |= 0x17;            /* make all row pins output */
    P5->OUT &= ~0x17;           /* drive all row pins low */
    delay();                    /* wait for signals to settle */
    col = P5->IN & 0xE0;        /* read all column pins */
    P5->OUT |= 0x17;            /* drive all rows high before disable them */
    P5->DIR &= ~0x17;           /* disable all row pins drive */
    if (col == 0xE0)            /* if all columns are high */
    ...
    for (row = 0; row < 4; row++) {
        ...
        col = P5->IN & 0xE0;            /* read all columns */
        P5->OUT |= row_select[row];     /* drive the active row high */
        if (col != 0xE0) break;         /* if one of the input is low, some key is pressed. */
    }
    P5->OUT |= 0x17;                    /* drive all rows high before disable them */
    P5->DIR &= 0x17;                    /* disable all rows */
    if (row == 4)
